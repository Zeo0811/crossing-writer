...
import { HERO_PROJECT_ID, HERO_STATUSES, type HeroStatus, type MockProject } from "../fixtures/projects";
import { PHASES, phaseIndexOf, statusBadge as statusTone } from "../fixtures/phases";
import { PixelEmptyArt } from "../components/PixelIcons";

type Tab = "active" | "archived";

function statusLabel(s: HeroStatus): string {
  return HERO_STATUSES.find((x) => x.id === s)?.label ?? s;
}
...
export function MockProjectList() {
  ...
  const visible = useMemo(() => {
    return allProjects.filter((p) => {
      ...
      if (q && !`${p.name} ${p.product ?? ""}`.toLowerCase().includes(q.toLowerCase())) return false;
      return true;
    });
  }, [allProjects, tab, q, archivedIds]);

  const activeCount = allProjects.filter((p) => !archivedIds.has(p.id)).length;
  const archivedCount = archivedIds.size;

  function archive(id: string) {
    setArchivedIds((s) => new Set(s).add(id));
    setMenuId(null);
    m.pushToast({ type: "success", message: "项目已归档" });
  }
  function restore(id: string) {
    setArchivedIds((s) => { const n = new Set(s); n.delete(id); return n; });
    m.pushToast({ type: "success", message: "项目已恢复" });
  }
  function confirmDelete(p: MockProject) {
    setDeleteTarget(p);
    setMenuId(null);
  }
  function destroy(p: MockProject) {
    setArchivedIds((s) => { const n = new Set(s); n.delete(p.id); return n; });
    setDeleteTarget(null);
    m.pushToast({ type: "success", message: "项目已删除" });
  }
  function open(p: MockProject) {
    nav(`/mock/projects/${p.id}`);
  }

  return (
    <div onClick={() => setMenuId(null)}>
      <header className="flex items-center justify-end mb-[18px]">
        <button
      ...
      <div className="flex items-center gap-3 mb-[18px]">
        ...
        <div className="flex-1 relative">
          ...
          {q && (
            <button onClick={() => setQ("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--faint)] hover:text-[var(--heading)] text-xs leading-none">✕</button>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-[18px]">
          {visible.map((p) => (
            <ProjectCard
              key={p.id}
              project={p}
              archived={archivedIds.has(p.id)}
              isHero={p.id === HERO_PROJECT_ID}
      ...
      {showNew && (
        <NewProjectModal
          ...
          onCreate={(name) => {
            setShowNew(false);
            m.pushToast({ type: "success", message: "项目已创建" });
            nav(`/mock/projects/${HERO_PROJECT_ID}`);
          }}
        />
      )}
      {deleteTarget && (
    ...
    <button
      ...
      className={`px-3 py-1.5 rounded text-xs ${
        active
          ...
          : "text-[var(--meta)] hover:text-[var(--heading)] border border-transparent"
      }`}
    >
      {children}
    </button>
...
function ProjectCard({
  ...
  const phase = phaseIndexOf(p.status);
  return (
    <article
      onClick={onOpen}
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-[var(--meta)]">
            {p.product ?? "—"} · {timeAgo(p.updated_at)}
          </div>
        </div>
        <span
      ...
      <div className="text-[11px] text-[var(--meta)]">{statusLabel(p.status)}</div>
      ...
      {menuOpen && (
        ...
        >
          ...
          <MenuItem onClick={() => alert("演示：重命名（Checkpoint 1.5 接入）")}>重命名</MenuItem>
          {archived ? (
        ...
        <div className="p-4">
          <label className="block">
            ...
            <input
              ...
              placeholder="例：测评 Lovable Studio"
...
function DeleteModal({ project, onCancel, onConfirm }: { project: MockProject; onCancel: () => void; onConfirm: () => void }) {
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-[rgba(0,0,0,0.5)] backdrop-blur-sm" onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-[440px] rounded border border-[var(--red)] bg-[var(--bg-1)] shadow-2xl">
        <div className="px-4 py-3 border-b border-[var(--hair)]">
          <div className="text-[10px] tracking-[2px] text-[var(--red)]" style={{ fontFamily: "var(--font-pixel)" }}>DANGER</div>
          <h3 className="text-[var(--heading)] mt-0.5">永久删除项目？</h3>
        </div>
        <div className="p-4 space-y-3">
          <p className="text-sm text-[var(--body)]">
            这会**永久删除** <span className="text-[var(--red)] font-semibold">{project.name}</span> 及其所有 brief、case、evidence、稿件。此操作不可恢复。
          </p>
          <label className="block">
