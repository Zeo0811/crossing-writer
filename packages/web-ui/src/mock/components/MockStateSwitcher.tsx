  ...
  if (!m.switcherOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 z-30 px-3 py-2 rounded border border-[var(--accent-soft)] bg-[var(--bg-1)] text-[var(--accent)] text-xs hover:bg-[var(--accent-fill)]"
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3 py-2 bg-[var(--accent-fill)] border-b border-[var(--accent-soft)]">
        <span
          className="text-[11px] tracking-[1px] text-[var(--accent)]"
          style={{ fontFamily: "var(--font-pixel)" }}
        >
          MOCK PANEL
        </span>
      ...
      <div className="p-3 space-y-3">
        <div>
          <div className="text-[10px] uppercase tracking-wider text-[var(--meta)] mb-1.5">Hero project status</div>
          <select
            value={m.heroStatus}
        ...
        <div>
          <div className="text-[10px] uppercase tracking-wider text-[var(--meta)] mb-1.5">CLI health</div>
        ...
        <div>
          <div className="text-[10px] uppercase tracking-wider text-[var(--meta)] mb-1.5">Toast 演示</div>
          <div className="flex gap-1">
            ...
            <button onClick={() => m.pushToast({ type: "info", message: "正在重新解析 Brief…" })} className="flex-1 px-2 py-1 text-[11px] rounded border border-[var(--amber-hair)] text-[var(--amber)] hover:bg-[var(--amber-bg)]">info</button>
          </div>
        </div>
        <div className="text-[10px] text-[var(--faint)] leading-relaxed">
          ⌘K 唤起命令面板。Hero 状态会驱动 Workbench 各段视图（在后续 checkpoint 接入）。
        </div>
      </div>
    </div>
  );
